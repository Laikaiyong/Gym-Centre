<%-- 
    Document   : feedback
    Created on : 23 May 2024, 5:44:40 pm
    Author     : vandycklai
--%>
...
<%
            List<Customer> oriCustomers = (List<Customer>) request.getAttribute("customers");
    Trainer editingUser = (Trainer) session.getAttribute("user");
    List<Feedback> feedbacks = editingUser.getFeedbacks();
    ...
    try {
        for (GymClass gym: gymClasses) {
            if (!gym.getComments().isEmpty()) {
                comments.addAll(gym.getComments());
            }
        }
    } catch (Exception e) {
        e.printStackTrace();
...
try {
        for (Customer cust: oriCustomers) {
            for (GymClass gym: cust.getClasses()) {
                for (GymClass gym1: gymClasses) {
                    if (gym.getId() == gym1.getId()) {
                            customersId.add(cust.getUsername());
                            customers.add(cust);
                    }
                }
            }
...
<section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
    ...
    <div class="mx-auto max-w-screen-xl px-4 lg:px-12">
        <h1 class="mb-4 text-2xl font-extrabold tracking-tight leading-none text-gray-900 md:text-3xl lg:text-6xl dark:text-white">Feedback</h1>
...
<section class="bg-gray-50 dark:bg-gray-900 p-3 sm:p-5">
  ...
  <th scope="col" class="px-4 py-3">Gender</th>
                    ...
                    <tbody>
                        <tr class="border-b dark:border-gray-700">
                            <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">16</th>
                            <td class="px-4 py-3">customer1</td>
<td class="px-4 py-3"> Male</td>
                            <td class="px-4 py-3">Happy Hour</td>
                            <td class="px-4 py-3">Japan</td>
                            <td class="px-4 py-3">21</td>
                            <td class="px-4 py-3">0199999999</td>
                            <td class="px-4 py-3 flex items-center justify-end">
                            </td>
                        </tr>
...
<td class="px-4 py-3"> <%= customer.getGender() %></td>
    ...
    <% for (GymClass gym : gymClasses) { %>
      <tr class="border-b dark:border-gray-700">
                            ...
                            <td class="px-4 py-3 flex items-center justify-end">
                                <%
                                if (session.getAttribute("userRole").toString().equals("trainer")  && gym.getFeedback().isEmpty()) {
                            %><a href="addFeedback?id=<%= gym.getId() %>" class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Add Feedback</a><%}%>
                            </td>
                        </tr>
    <% } %>
        ...
        <%
                                             if (session.getAttribute("addGym") != null && session.getAttribute("userRole").toString().equals("trainer")) {
                            %>
                        ...
                        <h1 class="text-xl font-bold leading-tight tracking-tight text-gray-900 md:text-2xl dark:text-white">
                            ...
                            GymClass gym = (GymClass) session.getAttribute("addGym");
                                %>
                        <form class="space-y-4 md:space-y-6" action="addFeedback" method="POST" >
                            ...
                            <button type="submit" value="addFeedback" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Add</button>
                            <%
                                if (session.getAttribute("addFeedbackError") != null) {
                            %>
                            <div id="alert-additional-content-2" class="p-4 mb-4 text-red-800 border border-red-300 rounded-lg bg-red-50 dark:bg-gray-800 dark:text-red-400 dark:border-red-800" role="alert">
                                ...
                                <div class="mt-2 mb-4 text-sm">
                                    <%=session.getAttribute("addFeedbackError")%>
                                </div>
                                <div class="flex">
